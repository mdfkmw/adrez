...
router.post('/owner/set-active', async (req, res) => {
  try {
    ...
    const agent_id = req.body?.agent_id ? Number(req.body.agent_id) : null;
...
router.get('/:id/report', async (req, res) => {
  ...
  try {
    ...
    // 2️⃣ Rezervări + loc + segment (ID + nume stații) + creat la + cine a rezervat  
...
router.put('/:id', async (req, res) => {
  ...
  const phone = (req.body?.phone ?? '').toString().replace(/\D/g, '') || null;
  const notes = (req.body?.notes ?? '').toString();
  try {
    if (!id) return res.status(400).json({ error: 'id lipsă' });
    // telefonul poate rămâne neschimbat dacă e null
    await db.query(
      `UPDATE people
