  ...
  try {
    ...
    const tripsSql = `
      SELECT
        ...
        COUNT(res.id) AS seats_booked,
        COALESCE(SUM(rd.discount_amount), 0) AS discount_total,
        COUNT(rd.id) AS discount_count,
      ...
      LEFT JOIN reservations res ON res.trip_id = t.id AND res.status NOT IN ('cancelled', 'cancelled')
    ...
    const summarySql = `
      SELECT
        COUNT(res.id) AS total_seats_booked,
        SUM(CASE WHEN res.status IN ('cancelled', 'cancelled') THEN 1 ELSE 0 END) AS total_cancels_noshow,
        SUM(CASE WHEN pay.id IS NOT NULL THEN 1 ELSE 0 END) AS paid_seats,
        COALESCE(SUM(pay.amount), 0) AS paid_total,
        COALESCE(SUM(CASE WHEN pay.id IS NOT NULL THEN rd.discount_amount ELSE 0 END), 0) AS paid_discounts,
        SUM(CASE WHEN pay.id IS NULL THEN 1 ELSE 0 END) AS reserved_seats,
        COALESCE(SUM(CASE WHEN pay.id IS NULL THEN rp.price_value ELSE 0 END), 0) AS reserved_total,
        COALESCE(SUM(CASE WHEN pay.id IS NULL THEN rd.discount_amount ELSE 0 END), 0) AS reserved_discounts
      FROM trips t
      JOIN route_schedules rs ON rs.id = t.route_schedule_id
    ...
    const discountsByTypeSql = `
      SELECT
        ...
        SUM(CASE WHEN pay.id IS NOT NULL THEN 1 ELSE 0 END) AS paid_count,
        COALESCE(SUM(CASE WHEN pay.id IS NOT NULL THEN rd.discount_amount ELSE 0 END), 0) AS paid_total,
        SUM(CASE WHEN pay.id IS NULL THEN 1 ELSE 0 END) AS reserved_count,
        COALESCE(SUM(CASE WHEN pay.id IS NULL THEN rd.discount_amount ELSE 0 END), 0) AS reserved_total
      FROM trips t
      JOIN route_schedules rs ON rs.id = t.route_schedule_id
      LEFT JOIN reservations res ON res.trip_id = t.id
      LEFT JOIN reservation_discounts rd ON rd.reservation_id = res.id
