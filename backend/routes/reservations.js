...
// cache local pentru stații — previne interogări repetate
...
async function touchTravelerDefault(phone, routeId, boardStationId, exitStationId) {
  try {
    const cleanPhone = sanitizePhone(phone);
    if (!cleanPhone || !Number.isInteger(routeId) || !Number.isInteger(boardStationId) || !Number.isInteger(exitStationId)) {
      return;
    }
      ...
      await db.query(
        `
          UPDATE traveler_defaults
           ...
           WHERE id = ?
        `,
        [boardStationId, exitStationId, existing.rows[0].id],
      );
    } else {
      await db.query(
        `
          INSERT INTO traveler_defaults
            (phone, route_id, board_station_id, exit_station_id, use_count, last_used_at)
          VALUES
            (?, ?, ?, ?, 1, NOW())
        `,
        [cleanPhone, routeId, boardStationId, exitStationId],
      );
    }
  } catch (err) {
...
router.post('/', async (req, res) => {
  ...
  const {
    ...
    price_list_id,
    booking_channel,
    pay_cash_now = false,      // dacă e true și passenger.payment_method==='cash', vei tipări după creare (fluxul tău existent)
    ...
    for (const p of passengers) {
      ...
      if (p.reservation_id) {
        ...
        await db.query(
          ...
          WHERE id = ?
          `,
          [person_id, p.seat_id, boardStationId, exitStationId, p.observations || null, p.reservation_id]
        );

        await touchTravelerDefault(phone, Number(route_id), boardStationId, exitStationId);
        await logEvent(p.reservation_id, 'update', Number(req.user?.id) || null, {
          person_id, seat_id: p.seat_id, board_station_id: boardStationId, exit_station_id: exitStationId
        });
        ...
        const insRes = await db.query(
          ...
          VALUES (?, ?, ?, ?, ?, ?, 'active', ?)
          `,
          [trip_id, p.seat_id, person_id, boardStationId, exitStationId, p.observations || null, Number(req.user?.id) || null]
        );
        ...
        await logEvent(newResId, 'create', Number(req.user?.id) || null, {
          trip_id, seat_id: p.seat_id, person_id, board_station_id: boardStationId, exit_station_id: exitStationId
        });
...
router.post('/:id/payments/cash', async (req, res) => {
  try {
    ...
    await logEvent(reservationId, 'pay', Number(employeeId) || Number(req.user?.id) || null, { method: 'cash', amount });
      ...
      try {
        ...
        if (rows.length) {
          ...
          if (Number.isInteger(routeId) && routeId > 0) {
            await touchTravelerDefault(row.phone, routeId, boardValue, exitValue);
          }
        }
      } catch (err) {
