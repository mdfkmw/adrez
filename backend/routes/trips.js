  ...
  try {
    ...
    for (let d = 0; d < 7; d++) {
      ...
      for (const s of schedules) {
        ...
        // UPSERT atomic ca în PG (ne bazăm pe unique key uq_trips_route_date_time_vehicle)
        ...
        const { rows: trip } = await db.query(
          `SELECT id FROM trips WHERE route_id = ? AND date = ? AND time = ? AND vehicle_id = ? LIMIT 1`,
          [s.route_id, dateStr, s.departure, defaultVehicleId]
        );
...
router.post('/exceptions/update', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // 3️⃣ Manual upsert (MariaDB) cu NULL-safe equality <=> 
      ...
      if (findRes.length) {
        // UPDATE
        const disableRunVal = (runFlag ?? false) ? 1 : 0;
        const disableOnlineVal = (onlineFlag ?? false) ? 1 : 0;
        await conn.query(
