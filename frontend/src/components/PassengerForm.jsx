...
const PassengerForm = ({
    ...
    const updateSegmentForSeat = (prevState, rawBoard, rawExit, options = {}) => {
        ...
        const anchorSeat = options.anchorSeat
            || selectedSeats.find((s) => {
                if (s.id === seat.id) return false;
                const data = prevState[s.id];
    ...
    useEffect(() => {
        ...
        fetch(`/api/traveler-defaults?phone=${encodeURIComponent(digits)}&route_id=${selectedRoute.id}`)
            .then(res => {
            ...
            .then(data => {
                ...
                setPassengersData(prev => {
                    const updated = updateSegmentForSeat(prev, boardName, exitName);
                    updated[seat.id] = { ...(updated[seat.id] || {}), segmentAutoAppliedPhone: digits };
                    return updated;
                });
            })
         ...
         shadow-lg z-50 text-sm whitespace-normal">
            ...
            {segmentNotice?.type === 'history' && (
                <div className="text-xs text-red-700 bg-red-50 border border-red-200 rounded px-3 py-2 mb-2">
                    Segment actualizat automat (istoric client): <strong>{segmentNotice.board || defaultBoard}</strong> →{' '}
                    <strong>{segmentNotice.exit || defaultExit}</strong>
                </div>
            )}
            <div className="flex gap-4">
                <Select
