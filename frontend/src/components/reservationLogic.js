...
function bestTriple(rows, b, e, stops) {
  ...
  if (pair) {
    ...
    if (singles.length) {
      const cand = { seats:[...pair.seats, singles[0].seat], score: pair.score + singles[0].score, type:'pair+single' };
      best = cand;
    }
  }
...
export function getBestAvailableSeat(seats, board_at, exit_at, stops, excludeIds = [], options = {}) {
...
export function getBestSeatCandidate(seats, board_at, exit_at, stops, excludeIds = [], options = {}) {
  const filtered = (seats || []).filter((s) => !excludeIds?.includes?.(s?.id));
  const list = selectSeats(filtered, board_at, exit_at, stops, 1);
  const chosen = list[0] || null;
