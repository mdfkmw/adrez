...
const AdminDiscountType = () => {
  ...
  const fetchDiscounts = async () => {
    const res = await axios.get('http://localhost:5000/api/discount-types');
  ...
  const handleSave = async () => {
    ...
    if (editingDiscount) {
      await axios.put(
        `http://localhost:5000/api/discount-types/${editingDiscount.id}`,
        newDiscount
      );
    } else {
      await axios.post('http://localhost:5000/api/discount-types', newDiscount);
    }
