...
const unsettledTotal = unsettled.reduce((s, r) => s + Number(r.total_amount || 0), 0);
  ...
  useEffect(() => {
    ...
    fetch(`/api/routes?operator_id=${operatorId}&date=${range.start}`)
      .then(r => r.json())
      .then(routes => {
        setRouteOptions(
          routes.map(r => ({
            id: r.id,
            name: r.name,
            hours: Array.isArray(r.schedules)
              ? r.schedules.map(s => s.departure) // "HH:MM"
              : []
          }))
        );
      })
  ...
  //Când se schimbă ruta aleasă, calculează orele disponibile
  useEffect(() => {
    if (!selectedRoute) {
      setHourOptions([]);
    ...
    // Reset ora când se schimbă ruta
    setSelectedHour(null);
