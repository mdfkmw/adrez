...
export default function RouteEditorPage() {
  ...
  const saveRoute = async () => {
    ...
    const payload = stops.map((s,i)=> ({
      ...s,
      sequence: i+1,
      geofence_polygon: s.geofence_type === "polygon" ? (s.geofence_polygon ?? null) : null,
      geofence_radius_m: s.geofence_type === "circle"  ? (s.geofence_radius_m ?? 0) : null,
    }));
  ...
  const handleMapLoad = (map) => {
    ...
    if (stops.length && !didFitOnce.current) {
      ...
      stops.forEach(s => b.extend(new window.google.maps.LatLng(s.latitude, s.longitude)));
      map.fitBounds(b);
    ...
    <div className="flex">
      ...
      <main className="flex-1 relative">
        ...
        {!mapsReady ? (
          ...
          >
            {/* traseu */}
            {stops.length > 1 && <Polyline path={stops.map(s=>({lat:s.latitude, lng:s.longitude}))} options={{ strokeWeight: 3 }} />}
